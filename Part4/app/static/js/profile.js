...
async function saveChanges() {
    const emailInput = document.getElementById('emailInput');
    const token = getCookie('token'); // الحصول على التوكن من الكوكيز
    ...
    if (response.ok) {
        showNotification('تم تحديث البيانات في قاعدة البيانات', 'success');
        emailInput.setAttribute('readonly', 'readonly');
    } else {
        showNotification('فشل التحديث في السيرفر', 'error');
    }
}
// إلغاء التعديل
function cancelEdit() {
